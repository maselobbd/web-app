...
if((process.env.NODE_ENV).toLowerCase() === "development") {
  ...
  config = {
    ...
    driver: process.env.LOCAL_SQLDRIVER, // Required if using Windows Authentication
  };
} else {
  sql = require("mssql");
...
const createTVP = (objects, columns, tvpName) => {
    const table = new sql.Table(tvpName);
    table.create = true
    columns.forEach(column => {
        if (column.length) {
            table.columns.add(column.name, mapTypes(column.type), column.length);
        } else {
            table.columns.add(column.name, mapTypes(column.type));
        }
    });

    objects.forEach(item => {
        const row = columns.map(column => item[column.name]);
        table.rows.add(...row);
    });
